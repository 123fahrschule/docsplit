...
module Docsplit
  ...
  METADATA_KEYS = %i[author date creator keywords producer subject title length]

  GM_FORMATS = ['image/gif', 'image/jpeg', 'image/png', 'image/x-ms-bmp', 'image/svg+xml', 'image/tiff',
                'image/x-portable-bitmap', 'application/postscript', 'image/x-portable-pixmap']

  DEPENDENCIES = { java: false, gm: false, pdftotext: false, pdftk: false, pdftailor: false,
                   tesseract: false, osd: false }
